...
//hash2
int hash2(char str[], int a[])
{
	int i, h;
	for (i = 0, h = 0; i < strlen(str); i++)
	{
		h += a[i] * (int)str[i]; //Sigma a[i]*str[i]
	}
	h = h % TABLE_SIZE;
	return h;
}
...
//hash2 입력
int hash_insert2(char name[], int a[])
{
	int number = 0; // for collision
	int key = hash2(name, a); //hash1함수 사용
...
int main()
{
	...
	int a[10]; //10개의 랜덤값 저장
	for (int i = 0; i < 10; i++)
	{
		a[i] = rand() % (TABLE_SIZE - 1) + 1; //랜덤값저장
	}
	...
	{
		hash_insert2(input2, a); //해쉬테이블1 입력
	}
	fclose(fp2);
